...
class SAGEConvolutions(nn.Module):
    ...
    def bn(self, i, x):
        ...
        x = x.view(-1, num_channels)  # reshape
...
class DiffPoolLayer(nn.Module): # with SAGE is the unit of the DiffPool layer
    ...
    def __init__(self, dim_input, dim_hidden, dim_embedding, current_num_clusters,
        ...
        if pooling_type != 'gnn':  # init matrix
            if self.invariant:
