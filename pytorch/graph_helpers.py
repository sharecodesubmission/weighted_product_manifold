...
def djikstra_wrapper(_x):
    (mat, x) = _x  # _x are sparse matrix of the graph, chunk range
    # apply dijkstra to find the shortest path between the points in the graphs
    # the result will be the dis_matrix with shape (n_indices, n_node) and the dis_matrix(i,j) give the shortest
...
def build_distance(G, scale, num_workers=None):
    ...
    # tao mot ma tran ke la ma tran thua
    adj_mat_original = nx.to_scipy_sparse_matrix(G, nodelist=list(range(G.order())))

    # Simple chunking (chia graph ra thanh cac manh nho de tinh djikstra distance)
    ...
    if n > nChunks:
        ...
        # get final matrix H
        # p.map will apply function djikstra_wrapper for each the element of list and also return a list of the results
        # Hs is the [(n_indices, n_node),...(n_indices_last, n_node]
        Hs = p.map(djikstra_wrapper, [(adj_mat_original, chunk) for chunk in chunks])
        H = np.concatenate(Hs, 0)
        logging.info(f"\tFinal Matrix {H.shape}")  # will have n_node x n_node
    else:
