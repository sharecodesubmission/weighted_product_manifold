...
class ProductEmbedding(nn.Module):
    def __init__(self, n, hyp_d, hyp_copies=1, euc_d=1, euc_copies=0, sph_d=1, sph_copies=0, project=True,
                 initialize=None, initial_scale=0.0, absolute_loss=False, logrel_loss=False,
                 dist_loss=False, square_loss=False, sym_loss=False, riemann=False):
        super().__init__()
        self.n = n
        self.riemann = riemann

        self.H = nn.ModuleList([Embedding(HyperboloidParameter.dist_h, HyperboloidParameter, n, hyp_d, project,
                                          initialize, initial_scale) for _ in range(hyp_copies)])
        self.E = nn.ModuleList(
            [Embedding(dist_e, EuclideanParameter, n, euc_d, initialize, False, initial_scale) for _ in
             range(euc_copies)])

        self.S = nn.ModuleList(
            [Embedding(dist_s, SphericalParameter, n, sph_d, project, initialize, initial_scale) for _ in
             range(sph_copies)])

        # list scale_params
        self.scale_params = []
        # list cac weight from ModuleList
        ...
        exp_str = f"Exponential No Rescale"
    ...
    def loss(self, _x, weights):
        idx, values, w = _x
        # idx is the tensor with the id of the vertices
        # values is the 1-d tensor with the scores
        # w is the rescale
        d = self.dist_idx(idx, weights)  # distance in embedding
...
class Embedding(nn.Module):
    def __init__(self, dist_fn, param_cls, n, d, project=True, initialize=None, initial_scale=0.0):
        super().__init__()
        self.dist_fn = dist_fn
        self.n, self.d = n, d
        self.project = project
        if initialize is not None: logging.info(
            f"Initializing {np.any(np.isnan(initialize.numpy()))} {initialize.size()} {(n, d)}")
